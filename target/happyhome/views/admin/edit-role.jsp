...
<%
    ...
    String message = (String) request.getAttribute("messageResponse");
            ...
            <div class="container-fluid">
                <div class="row">
                                        ...
                                        <div style="display: flex" class="row">
                                            <div class="form-group col-md-6">
                                                <div class="form-group">
                                                    <span>Tên quyền</span>
                                                    <input id="role_name" name="title" type="text" class="form-control" placeholder="" value="<%=role.getName()%>">
                    ...
                    <div class="card-footer row" style="width: 100%;">
                        <button type="submit" class="btn btn-primary" id="add-role-btn">Lưu </button>
                    </div>
                </div>
                <!-- /.row -->
            </div><!-- /.container-fluid -->
...
<script>
        <%
        for (Permission permiss : role.getPermission()){ %>
            $('#permiss input[value=<%=permiss.getId()%>]').prop('checked', true);
        <%}%>
    ...
    function addRole(data,actionUrl){
        $.ajax({
            ...
            success: function (result){
                window.location.href = "${ROLEurl}?action=edit&id=<%=role.getId()%>&message=update_success";
            },
            error: function (error){
                console.log(error);
                window.location.href = "${ROLEurl}?action=edit&id=<%=role.getId()%>&message=error_system";
            }
        });
    }
